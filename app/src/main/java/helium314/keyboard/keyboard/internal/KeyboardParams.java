    ...
    public void readAttributes(final Context context, @Nullable final AttributeSet attr) {
        ...
        try {
            ...
            if (touchPositionResId != 0) {
                // The historical sw600dp resource overrode the phone calibration with an empty
                // array. Select the same default data explicitly instead, so Android's resource
                // array contract stays consistent without a lint suppression.
                final boolean useAlphabetCalibration = mId.isAlphabetKeyboard()
                        && !Settings.getInstance().isTablet();
                final int actualId = useAlphabetCalibration
                        ? touchPositionResId : R.array.touch_position_correction_data_default;
