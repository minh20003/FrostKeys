...
class SparkleDustView @JvmOverloads constructor(
    ...
    private var particleBudget = FULL_PARTICLE_BUDGET
    ...
    /**
     * Caps both dust layers together. The caller resolves the budget from the user's effect mode
     * and current device constraints before beginning an animation.
     */
    fun setParticleBudget(budget: Int) {
        val sanitized = budget.coerceIn(0, FULL_PARTICLE_BUDGET)
        if (particleBudget == sanitized) return
        particleBudget = sanitized
        ambientDust.clear()
        streamDust.clear()
        if (sanitized == 0) {
            stopDustAnimation()
    ...
    internal fun isDustAnimationRunningForTest(): Boolean = isAnimating
    ...
    private fun createParticles(width: Float, height: Float) {
        ...
        // Keep the entire effect in its selected 200/400-particle budget even on a large
        // keyboard. This avoids the former ~1,700-circle frame workload and makes cost stable.
        val totalCount = (particleBudget * areaScale).toInt().coerceIn(1, particleBudget)
        val ambientCount = (totalCount * 45) / 100
        val streamCount = totalCount - ambientCount
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (particleBudget > 0 && (ambientDust.isEmpty() || streamDust.isEmpty())) {
            createParticles(width.toFloat(), height.toFloat())
        }
        ...
        canvas.withClip(clipPath) {
            drawStreamDust(this, dt)
            drawAmbientDust(this, dt)
        }

        // The glow is decorative, so deliberately stay at 30fps instead of permanently
        // invalidating at the display refresh rate.
    ...
    override fun onVisibilityChanged(changedView: View, visibility: Int) {
        ...
        if (visibility != VISIBLE) {
            // The AI panel can be hidden while a request is still running. Do not keep a delayed
            // 30 fps invalidation loop alive until the panel is detached.
            stopDustAnimation()
        }
    }
    ...
    private companion object {
        ...
        private const val TARGET_FRAME_MS = 33.3333f
