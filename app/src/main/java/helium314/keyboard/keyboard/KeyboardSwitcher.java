...
import helium314.keyboard.keyboard.internal.TranslationView;
...
public final class KeyboardSwitcher implements KeyboardState.SwitchActions {
    ...
    private ViewStub mEmojiPalettesStub;
    private KlipyPalettesView mKlipyPalettesView;
    private ViewStub mKlipyPalettesStub;
    ...
    private ViewStub mClipboardHistoryStub;
    private boolean mClipboardHistoryPreferenceListenerRegistered;
    private AiWritingToolsView mAiWritingToolsView;
    private ViewStub mAiWritingToolsStub;
    private TranslationView mTranslationView;
    private ViewStub mTranslationStub;
    ...
    public void onHideWindow() {
        exitResizeMode();
        clearTransitions();
        // Hidden IME panels must not keep cloud calls, media work, or animations alive.
        closeLazyPanels();
        if (mKeyboardView != null) {
    ...
    private void configureEmojiPalettesView(@NonNull final EmojiPalettesView emojiPalettesView) {
        emojiPalettesView.setVisibility(View.GONE);
        emojiPalettesView.setHardwareAcceleratedDrawingEnabled(mIsHardwareAcceleratedDrawingEnabled);
        emojiPalettesView.setKeyboardActionListener(mLatinIME.mKeyboardActionListener);
    }

    @Nullable
    private EmojiPalettesView ensureEmojiPalettesView() {
        if (mEmojiPalettesView != null) {
            return mEmojiPalettesView;
        }
        if (mCurrentInputView == null || mEmojiPalettesStub == null) {
            return null;
        }
        try {
            final View inflated = mEmojiPalettesStub.inflate();
            mEmojiPalettesStub = null;
            if (!(inflated instanceof EmojiPalettesView)) {
                Log.w(TAG, "Emoji stub inflated an unexpected view");
                return null;
            }
            mEmojiPalettesView = (EmojiPalettesView) inflated;
            configureEmojiPalettesView(mEmojiPalettesView);
        } catch (IllegalStateException e) {
            // A duplicate open can race a stale ViewStub reference during input-view recreation.
            final View existing = mCurrentInputView.findViewById(R.id.emoji_palettes_view);
            if (existing instanceof EmojiPalettesView) {
                mEmojiPalettesView = (EmojiPalettesView) existing;
                configureEmojiPalettesView(mEmojiPalettesView);
            } else {
                Log.w(TAG, "Could not lazily inflate emoji panel", e);
            }
        }
        return mEmojiPalettesView;
    }

    private void configureClipboardHistoryView(@NonNull final ClipboardHistoryView clipboardHistoryView) {
        clipboardHistoryView.setVisibility(View.GONE);
        clipboardHistoryView.setHardwareAcceleratedDrawingEnabled(mIsHardwareAcceleratedDrawingEnabled);
        clipboardHistoryView.setKeyboardActionListener(mLatinIME.mKeyboardActionListener);
        if (!mClipboardHistoryPreferenceListenerRegistered && mCurrentInputView != null) {
            KtxKt.prefs(mCurrentInputView.getContext())
                    .registerOnSharedPreferenceChangeListener(clipboardHistoryView);
            mClipboardHistoryPreferenceListenerRegistered = true;
        }
    }

    @Nullable
    private ClipboardHistoryView ensureClipboardHistoryView() {
        if (mClipboardHistoryView != null) {
            return mClipboardHistoryView;
        }
        if (mCurrentInputView == null || mClipboardHistoryStub == null) {
            return null;
        }
        try {
            final View inflated = mClipboardHistoryStub.inflate();
            mClipboardHistoryStub = null;
            if (!(inflated instanceof ClipboardHistoryView)) {
                Log.w(TAG, "Clipboard stub inflated an unexpected view");
                return null;
            }
            mClipboardHistoryView = (ClipboardHistoryView) inflated;
            configureClipboardHistoryView(mClipboardHistoryView);
        } catch (IllegalStateException e) {
            // A duplicate open can race a stale ViewStub reference during input-view recreation.
            final View existing = mCurrentInputView.findViewById(R.id.clipboard_history_view);
            if (existing instanceof ClipboardHistoryView) {
                mClipboardHistoryView = (ClipboardHistoryView) existing;
                configureClipboardHistoryView(mClipboardHistoryView);
            } else {
                Log.w(TAG, "Could not lazily inflate clipboard panel", e);
            }
        }
        return mClipboardHistoryView;
    }
    ...
    @Nullable
    private AiWritingToolsView ensureAiWritingToolsView() {
        if (mAiWritingToolsView != null) {
            return mAiWritingToolsView;
        }
        if (mCurrentInputView == null || mAiWritingToolsStub == null) {
            return null;
        }
        try {
            final View inflated = mAiWritingToolsStub.inflate();
            mAiWritingToolsStub = null;
            if (!(inflated instanceof AiWritingToolsView)) {
                Log.w(TAG, "AI tools stub inflated an unexpected view");
                return null;
            }
            mAiWritingToolsView = (AiWritingToolsView) inflated;
            mAiWritingToolsView.setVisibility(View.GONE);
            mAiWritingToolsView.setHardwareAcceleratedDrawingEnabled(mIsHardwareAcceleratedDrawingEnabled);
            mAiWritingToolsView.setKeyboardActionListener(mLatinIME.mKeyboardActionListener);
            mAiWritingToolsView.updateThemeColors(Settings.getValues().mColors);
        } catch (IllegalStateException e) {
            // A duplicate open can race a stale ViewStub reference during input-view recreation.
            final View existing = mCurrentInputView.findViewById(R.id.ai_writing_tools_view);
            if (existing instanceof AiWritingToolsView) {
                mAiWritingToolsView = (AiWritingToolsView) existing;
            } else {
                Log.w(TAG, "Could not lazily inflate AI tools panel", e);
            }
        }
        return mAiWritingToolsView;
    }
    ...
    @Nullable
    private TranslationView ensureTranslationView() {
        if (mTranslationView != null) {
            return mTranslationView;
        }
        if (mCurrentInputView == null || mTranslationStub == null) {
            return null;
        }
        try {
            final View inflated = mTranslationStub.inflate();
            mTranslationStub = null;
            if (!(inflated instanceof TranslationView)) {
                Log.w(TAG, "Translation stub inflated an unexpected view");
                return null;
            }
            mTranslationView = (TranslationView) inflated;
            mTranslationView.setVisibility(View.GONE);
            mTranslationView.setHardwareAcceleratedDrawingEnabled(mIsHardwareAcceleratedDrawingEnabled);
            mTranslationView.setKeyboardActionListener(mLatinIME.mKeyboardActionListener);
            mTranslationView.updateThemeColors(Settings.getValues().mColors);
        } catch (IllegalStateException e) {
            // A duplicate open can race a stale ViewStub reference during input-view recreation.
            final View existing = mCurrentInputView.findViewById(R.id.translation_view);
            if (existing instanceof TranslationView) {
                mTranslationView = (TranslationView) existing;
            } else {
                Log.w(TAG, "Could not lazily inflate translation panel", e);
            }
        }
        return mTranslationView;
    }
    ...
    @Nullable
    private KlipyPalettesView ensureKlipyPalettesView() {
        if (mKlipyPalettesView != null) {
            return mKlipyPalettesView;
        }
        if (mCurrentInputView == null || mKlipyPalettesStub == null) {
            return null;
        }
        try {
            final View inflated = mKlipyPalettesStub.inflate();
            mKlipyPalettesStub = null;
            if (!(inflated instanceof KlipyPalettesView)) {
                Log.w(TAG, "Klipy stub inflated an unexpected view");
                return null;
            }
            mKlipyPalettesView = (KlipyPalettesView) inflated;
            mKlipyPalettesView.setVisibility(View.GONE);
            mKlipyPalettesView.setHardwareAcceleratedDrawingEnabled(mIsHardwareAcceleratedDrawingEnabled);
            mKlipyPalettesView.updateThemeColors(Settings.getValues().mColors);
        } catch (IllegalStateException e) {
            // A duplicate open can race a stale ViewStub reference during input-view recreation.
            final View existing = mCurrentInputView.findViewById(R.id.klipy_palettes_view);
            if (existing instanceof KlipyPalettesView) {
                mKlipyPalettesView = (KlipyPalettesView) existing;
            } else {
                Log.w(TAG, "Could not lazily inflate Klipy panel", e);
            }
        }
        return mKlipyPalettesView;
    }

    private void closeAiWritingToolsPanel() {
        if (mAiWritingToolsView != null) {
            mAiWritingToolsView.setVisibility(View.GONE);
    ...
    private void closeTranslationPanel() {
        if (mTranslationView != null) {
            mTranslationView.setVisibility(View.GONE);
            mTranslationView.onClose();
        }
    }

    private void stopKlipyPalettesPanel() {
        if (mKlipyPalettesView != null) {
    ...
    private void closeLazyPanels() {
        ...
        closeAiWritingToolsPanel();
        closeTranslationPanel();
        stopKlipyPalettesPanel();
    }

    private void closePanelsForInputViewRecreation() {
        if (mEmojiPalettesView != null) {
            mEmojiPalettesView.stopEmojiPalettes();
            mEmojiPalettesView.setVisibility(View.GONE);
        }
    ...
    public void resetKeyboardStateToAlphabet(final int currentAutoCapsState,
        ...
        if (isShowingAiWritingTools() || isShowingTranslation()) {
            return;
        }
    ...
    public void onPressKey(final int code, final boolean isSinglePointer,
        ...
        if (isShowingKlipyPalettes() || isShowingEmojiPalettes() || isShowingClipboardHistory()
                || isShowingAiWritingTools() || isShowingTranslation()) {
            return;
        }
    ...
    public void setEmojiKeyboard() {
        ...
        final EmojiPalettesView emojiPalettesView = ensureEmojiPalettesView();
        if (emojiPalettesView == null || mKeyboardView == null || mMainKeyboardFrame == null) {
            return;
        }
        transitionToPanel(emojiPalettesView, new Runnable() {
            @Override
            public void run() {
                updatePersistentEmojiRow();
                ...
                emojiPalettesView.startEmojiPalettes(mKeyboardView.getKeyVisualAttribute(),
    ...
    public void setClipboardKeyboard() {
        ...
        final ClipboardHistoryView clipboardHistoryView = ensureClipboardHistoryView();
        if (clipboardHistoryView == null || mKeyboardView == null || mMainKeyboardFrame == null) {
            return;
        }
        transitionToPanel(clipboardHistoryView, new Runnable() {
            @Override
            public void run() {
                updatePersistentEmojiRow();
                mMainKeyboardFrame.setVisibility(View.VISIBLE);
                ...
                // Start clipboard
                clipboardHistoryView.startClipboardHistory(mLatinIME.getClipboardHistoryManager(),
                        mKeyboardView.getKeyVisualAttribute(),
                        mLatinIME.getCurrentInputEditorInfo(), mLatinIME.mKeyboardActionListener);
                clipboardHistoryView.setVisibility(View.VISIBLE);
    ...
    public void setAiToolsKeyboard() {
        ...
        final AiWritingToolsView aiWritingToolsView = ensureAiWritingToolsView();
        if (aiWritingToolsView == null || mKeyboardView == null || mMainKeyboardFrame == null) {
            return;
        }
        transitionToPanel(aiWritingToolsView, new Runnable() {
            @Override
            public void run() {
                updatePersistentEmojiRow();
                mMainKeyboardFrame.setVisibility(View.VISIBLE);
                ...
                // Force the panel to match the keyboard frame, not the screen.
                android.widget.FrameLayout.LayoutParams lp = new android.widget.FrameLayout.LayoutParams(
                        android.view.ViewGroup.LayoutParams.MATCH_PARENT,
                        android.view.ViewGroup.LayoutParams.WRAP_CONTENT);
                aiWritingToolsView.setLayoutParams(lp);
                aiWritingToolsView.onOpen(mLatinIME.getCurrentInputConnection());
                aiWritingToolsView.setVisibility(View.VISIBLE);
                mStripContainer.setVisibility(View.GONE);
                setKeyboardPanelOffsets(true);
    ...
    public void setTranslationKeyboard() {
        if (DEBUG_ACTION) {
            Log.d(TAG, "setTranslationKeyboard");
        }
        final TranslationView translationView = ensureTranslationView();
        if (translationView == null || mKeyboardView == null || mMainKeyboardFrame == null) {
            return;
        }
        transitionToPanel(translationView, new Runnable() {
            @Override
            public void run() {
                updatePersistentEmojiRow();
                mMainKeyboardFrame.setVisibility(View.VISIBLE);
                ...
                android.widget.FrameLayout.LayoutParams lp = new android.widget.FrameLayout.LayoutParams(
                        android.view.ViewGroup.LayoutParams.MATCH_PARENT,
                        android.view.ViewGroup.LayoutParams.WRAP_CONTENT);
                translationView.setLayoutParams(lp);
                translationView.onOpen(mLatinIME.getCurrentInputConnection());
    ...
    public void setKlipyKeyboard() {
        ...
        final KlipyPalettesView klipyPalettesView = ensureKlipyPalettesView();
        if (klipyPalettesView == null || mKeyboardView == null || mMainKeyboardFrame == null) {
            return;
        }
        transitionToPanel(klipyPalettesView, new Runnable() {
            @Override
            public void run() {
                updatePersistentEmojiRow();
                mMainKeyboardFrame.setVisibility(View.VISIBLE);
                ...
                klipyPalettesView.startKlipyPalettes(
                        mKeyboardView.getKeyVisualAttribute(),
                        mLatinIME.getCurrentInputEditorInfo(),
                        mLatinIME.mKeyboardActionListener);
                klipyPalettesView.setVisibility(View.VISIBLE);
                mStripContainer.setVisibility(View.GONE);
                setKeyboardPanelOffsets(true);
    ...
    public enum KeyboardSwitchState {
        ...
        TRANSLATION(KeyboardId.ELEMENT_TRANSLATION),
        ACCESS_POINT(-1),
        KLIPY(-1),
        OTHER(-1);
    ...
    public KeyboardSwitchState getKeyboardSwitchState() {
        boolean hidden = !isShowingEmojiPalettes() && !isShowingClipboardHistory() && !isShowingAiWritingTools()
                && !isShowingTranslation() && !isShowingAccessPointMenu() && !isShowingKlipyPalettes()
        ...
        if (hidden) {
            ...
            return KeyboardSwitchState.TRANSLATION;
    ...
    public void onToggleKeyboard(@NonNull final KeyboardSwitchState toggleState) {
        ...
        if (currentState == toggleState) {
            ...
            if (toggleState == KeyboardSwitchState.EMOJI) {
                ...
                setAiToolsKeyboard();
            } else if (toggleState == KeyboardSwitchState.TRANSLATION) {
                setTranslationKeyboard();
                ...
                if (currentState == KeyboardSwitchState.CLIPBOARD || currentState == KeyboardSwitchState.EMOJI
                        || currentState == KeyboardSwitchState.AI_TOOLS || currentState == KeyboardSwitchState.TRANSLATION
                        || currentState == KeyboardSwitchState.KLIPY) {
    ...
    public void reloadMainKeyboard() {
        ...
        final boolean wasTranslation = isShowingTranslation();
    ...
    public boolean isShowingTranslation() {
        return mTranslationView != null && mTranslationView.getVisibility() == View.VISIBLE;
    }
    ...
    public View onCreateInputView(@NonNull Context displayContext, final boolean isHardwareAcceleratedDrawingEnabled) {
        // The previous InputView may still own lazily inflated panels while a theme/configuration
        // change replaces it. Stop their work before dropping references to that view hierarchy.
        closePanelsForInputViewRecreation();
        mIsHardwareAcceleratedDrawingEnabled = isHardwareAcceleratedDrawingEnabled;
        if (mKeyboardView != null) {
        ...
        mClipboardHistoryPreferenceListenerRegistered = false;
        ...
        mClipboardHistoryView = null;
        mEmojiPalettesStub = mCurrentInputView.findViewById(R.id.emoji_palettes_stub);
        mClipboardHistoryStub = mCurrentInputView.findViewById(R.id.clipboard_history_stub);
        mAiWritingToolsView = null;
        mKlipyPalettesView = null;
        mTranslationView = null;
        mAiWritingToolsStub = mCurrentInputView.findViewById(R.id.ai_writing_tools_stub);
        mKlipyPalettesStub = mCurrentInputView.findViewById(R.id.klipy_palettes_stub);
        mTranslationStub = mCurrentInputView.findViewById(R.id.translation_stub);
    ...
    public void updatePersistentEmojiRow() {
        ...
        final boolean inPanel = state == KeyboardSwitchState.EMOJI || state == KeyboardSwitchState.CLIPBOARD
                || state == KeyboardSwitchState.AI_TOOLS || state == KeyboardSwitchState.TRANSLATION
                || state == KeyboardSwitchState.KLIPY
                ...
                || (mTranslationView != null && mTranslationView.getVisibility() == View.VISIBLE)
        ...
        removeText.setText(context.getString(R.string.persistent_emoji_remove_row));
    ...
    public void updateLiveFrostedGlassColors() {
        ...
        // Update TranslationView keys
        if (mTranslationView != null) {
            mTranslationView.updateThemeColors(colors);
        }
