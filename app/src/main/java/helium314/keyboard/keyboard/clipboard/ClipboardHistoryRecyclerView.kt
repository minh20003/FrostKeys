...
class ClipboardHistoryRecyclerView @JvmOverloads constructor(
    ...
    private val touchHelper = ItemTouchHelper(object : ItemTouchHelper.SimpleCallback(0, ItemTouchHelper.LEFT) {
        ...
        override fun onSwiped(viewHolder: ViewHolder, dir: Int) {
            ...
            // The DAO performs the SQLite/file delete on its serialized IO queue and then
            // delivers the matching adapter removal on main. Avoid an optimistic local removal:
            // its backing cache would otherwise still contain the entry until the IO task wins.
        }
