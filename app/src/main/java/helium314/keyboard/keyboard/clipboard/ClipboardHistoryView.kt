...
@SuppressLint("CustomViewStyleable") // Reads shared keyboard visual attributes from the theme.
class ClipboardHistoryView @JvmOverloads constructor(
    ...
    private var lastPinnedQuotaState = ClipboardDao.PinnedQuotaState.NONE
    /** Adapter count last announced to RecyclerView; initial DAO load arrives asynchronously. */
    private var reportedHistoryItemCount = 0

    init {
        val clipboardAttributes = context.obtainStyledAttributes(
            attrs, R.styleable.ClipboardHistoryView, defStyle, R.style.ClipboardHistoryView,
        )
        try {
            pinIconId = clipboardAttributes.getResourceId(
                R.styleable.ClipboardHistoryView_iconPinnedClip, 0,
            )
        } finally {
            clipboardAttributes.recycle()
        }

        val keyboardViewAttributes = context.obtainStyledAttributes(
            attrs, R.styleable.KeyboardView, defStyle, R.style.KeyboardView,
        )
        try {
            keyBackgroundId = keyboardViewAttributes.getResourceId(
                R.styleable.KeyboardView_keyBackground, 0,
            )
        } finally {
            keyboardViewAttributes.recycle()
        }
    }
    ...
    private fun setupBottomRowKeyboard(editorInfo: EditorInfo, listener: KeyboardActionListener) {
        val keyboardView = findViewById<MainKeyboardView>(R.id.clipboard_bottom_row_keyboard)
    ...
    fun startClipboardHistory(
        ...
        clipboardRecyclerView.apply {
            ...
            reportedHistoryItemCount = clipboardAdapter.itemCount
            val keyboardWidth = ResourceUtils.getKeyboardWidth(context, settings.current)
            layoutParams.width = keyboardWidth

            // set side padding
            val keyboardAttributes = context.obtainStyledAttributes(
                null, R.styleable.Keyboard, R.attr.keyboardStyle, R.style.Keyboard,
            )
            val (leftPadding, rightPadding) = try {
                val left = (keyboardAttributes.getFraction(R.styleable.Keyboard_keyboardLeftPadding,
                    keyboardWidth, keyboardWidth, 0f)
                        * settings.current.mSidePaddingScale).toInt()
                val right = (keyboardAttributes.getFraction(R.styleable.Keyboard_keyboardRightPadding,
                    keyboardWidth, keyboardWidth, 0f)
                        * settings.current.mSidePaddingScale).toInt()
                left to right
            } finally {
    ...
    override fun onClipboardHistoryReady() {
        if (!this::clipboardAdapter.isInitialized) return
        val currentCount = clipboardAdapter.itemCount
        when {
            currentCount > reportedHistoryItemCount -> {
                clipboardAdapter.notifyItemRangeInserted(reportedHistoryItemCount, currentCount - reportedHistoryItemCount)
            }
            currentCount < reportedHistoryItemCount -> {
                clipboardAdapter.notifyItemRangeRemoved(currentCount, reportedHistoryItemCount - currentCount)
            }
            currentCount > 0 -> clipboardAdapter.notifyItemRangeChanged(0, currentCount)
        }
        reportedHistoryItemCount = currentCount
    }

    override fun onPinnedClipboardQuotaChanged(state: ClipboardDao.PinnedQuotaState) {
        val previous = lastPinnedQuotaState
        lastPinnedQuotaState = state
        if (state.isQuotaReached && state != previous) {
            Toast.makeText(context, R.string.clipboard_pinned_quota_reached, Toast.LENGTH_LONG).show()
        }
    }
    ...
    override fun onSharedPreferenceChanged(prefs: SharedPreferences?, key: String?) {
        ...
        if (::clipboardHistoryManager.isInitialized && key == Settings.PREF_CLIPBOARD_HISTORY_PINNED_FIRST) {
            ...
            clipboardAdapter.notifyItemRangeChanged(0, clipboardAdapter.itemCount)
        }
    }
}
