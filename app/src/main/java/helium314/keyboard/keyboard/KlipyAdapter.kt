...
class KlipyAdapter(
    ...
    private fun getImageLoader(context: android.content.Context): ImageLoader {
        return sharedImageLoader ?: synchronized(KlipyAdapter::class.java) {
            sharedImageLoader ?: ImageLoader.Builder(context.applicationContext)
                // Coil normally constructs its own OkHttp client. Supplying the gated factory is
                // what keeps thumbnail loads subject to the same opt-in and cancellation rules as
                // search and media downloads.
                .callFactory(CloudRequestGate.callFactory(context.applicationContext, CloudFeature.KLIPY_MEDIA))
                .components {
                ...
                .memoryCache {
                    MemoryCache.Builder(context.applicationContext)
                        .maxSizeBytes(
                            KlipyImageCachePolicy.memoryCacheBytes(Runtime.getRuntime().maxMemory()),
                        )
                        .build()
                }
                .diskCache {
                    DiskCache.Builder()
                        .directory(File(context.cacheDir, "klipy-thumbnails"))
                        .maxSizeBytes(KlipyImageCachePolicy.DISK_CACHE_BYTES)
                        .build()
                }
...
class SubtleCircularProgressDrawable : Drawable(), Animatable, Runnable {
    ...
    override fun draw(canvas: Canvas) {
        ...
        canvas.withRotation(angle, cx, cy) {
            drawArc(rect, 0f, 270f, false, paint)
        }
    }
