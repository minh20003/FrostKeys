...
class KeyboardActionListenerImpl(private val latinIME: LatinIME, private val inputLogic: InputLogic) : KeyboardActionListener {
    ...
    override fun onKeyDown(keyCode: Int, keyEvent: KeyEvent): Boolean {
        ...
        if (event.isHandled) {
            // Optional CJK engines must see decoded physical keys before ordinary Latin
            // InputLogic turns Romaji/Pinyin into literal Latin text. The service returns false
            // for every other subtype and pass-through editor action, preserving this path.
            if (latinIME.onHardwareEvent(event)) return true
