...
class KlipyHistoryDao private constructor(context: Context) {
    ...
    fun addHistory(id: String, url: String, type: String, width: Int = 0, height: Int = 0, previewUrl: String? = null) {
        ...
        db.transaction {
            ...
            val existingPin = getPinState(this, id, type)
            delete(TABLE_NAME, "$COLUMN_ID = ? AND $COLUMN_TYPE = ?", arrayOf(id, type))
    ...
    fun pinHistory(id: String, type: String) {
        ...
        db.transaction {
            ...
            update(TABLE_NAME, values, "$COLUMN_ID = ? AND $COLUMN_TYPE = ?", arrayOf(id, type))
            demoteOverflowPins(this, type)
            pruneHistory(this, type)
        }
    }
