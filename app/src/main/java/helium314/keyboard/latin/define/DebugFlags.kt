...
object DebugFlags {
    ...
    fun init(context: Context) {
        ...
        // A crash stack can include editor text, file paths and framework diagnostics. The
        // personal release policy keeps only the bounded sanitized in-memory log, so persistent
        // crash-report capture is a debug-build diagnostic only (even if a release user toggles
        // the developer preference).
        if (BuildConfig.DEBUG) CrashReportExceptionHandler(context.applicationContext).install()
    }
}
