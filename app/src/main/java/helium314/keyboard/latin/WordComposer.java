...
public final class WordComposer {
    ...
    public boolean moveCursorByAndReturnIfInsideComposingWord(final int expectedMoveAmount) {
        // Telex/VNI retain a whole Vietnamese syllable as one composition so a later tone or
        // shape key can relocate its mark. They cannot safely represent an insertion point in
        // the middle of that retained buffer. Tell InputLogic to finish the composition before
        // it moves the editor cursor; subsequent input then edits at the real cursor position
        // instead of appending to the end of the old composing span.
        if ("vi_telex".equals(mCombiningSpec) || "vi_vni".equals(mCombiningSpec)) {
            return false;
        }
