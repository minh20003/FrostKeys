...
interface Colors {
    ...
    fun selectAndColorDrawable(attr: TypedArray, color: ColorType): Drawable {
        val fallback = requireNotNull(attr.getDrawable(R.styleable.KeyboardView_keyBackground)) {
            "KeyboardView_keyBackground is required"
        }.mutate()
...
class DynamicColors(context: Context, override val themeStyle: String, override val hasKeyBorders: Boolean, private var keyboardBackground: Drawable? = null) : Colors {
    ...
    override fun setBackground(view: View, color: ColorType) {
        ...
        when (color) {
            ...
            MAIN_BACKGROUND -> {
                val backgroundImage = rawKeyboardBackground
                if (backgroundImage != null) {
                    if (view.width > 0 && view.height > 0) {
                        if (preparedBackground == null || view.width != lastWidth || view.height != lastHeight) {
                            ...
                            preparedBackground = createCenterCroppedDrawable(backgroundImage, view.width, view.height, view.context.resources)
                        }
                        view.background = preparedBackground
                    } else {
                        view.background = NoIntrinsicSizeDrawable(backgroundImage)
                    }
                } else {
...
class DefaultColors (
    ...
    override fun haveColorsChanged(context: Context): Boolean {
        if (!isFrosted || systemAccent == null || systemNeutral == null) {
            return false
        }
...
private fun colorFilter(color: Int, mode: BlendModeCompat = BlendModeCompat.MODULATE): ColorFilter =
    requireNotNull(BlendModeColorFilterCompat.createBlendModeColorFilterCompat(color, mode)) {
        "The configured blend mode must be supported"
    }
...
class NoIntrinsicSizeDrawable(private val drawable: Drawable) : Drawable() {
    ...
    // `Drawable.getOpacity()` itself remains an abstract legacy API.  Do not call the equally
    // deprecated delegate getter merely to satisfy it: this wrapper can safely be treated as
    // translucent by the renderer.
...
private fun createCenterCroppedDrawable(
    ...
    return try {
        ...
        val outputBitmap = createBitmap(targetWidth, targetHeight, android.graphics.Bitmap.Config.ARGB_8888)
        ...
        val croppedDrawable = outputBitmap.toDrawable(resources)
