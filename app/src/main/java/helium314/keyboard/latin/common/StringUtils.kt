...
fun isEmoji(text: CharSequence): Boolean {
    ...
    return emoji.isSingleGrapheme && mightBeEmoji(emoji)
        && (emoji.matches(emoRegex) || isEmojiOmittedByLegacyRegex(emoji))
}

/**
 * The bundled emoji catalog includes Unicode additions omitted by the legacy generated regex.
 * Keep these exact sequences here until that generator is upgraded: U+1F6D9, U+1FA8B..U+1FA8D,
 * U+1FACC, U+1FADD, U+1FAEB, and the skin-tone-capable U+1FAF9..U+1FAFA.
 */
private fun isEmojiOmittedByLegacyRegex(text: String): Boolean {
    val codePoint = text.codePointAt(0)
    var offset = Character.charCount(codePoint)
    if (offset < text.length && text.codePointAt(offset) == 0xFE0F) {
        offset += Character.charCount(0xFE0F)
    }
    val supportsSkinTone = codePoint in 0x1FAF9..0x1FAFA
    if (supportsSkinTone && offset < text.length && text.codePointAt(offset) in 0x1F3FB..0x1F3FF) {
        offset += Character.charCount(text.codePointAt(offset))
    }
    if (offset != text.length) return false
    return codePoint == 0x1F6D9 || codePoint in 0x1FA8B..0x1FA8D ||
        codePoint == 0x1FACC || codePoint == 0x1FADD || codePoint == 0x1FAEB || supportsSkinTone
}
...
val String.isSingleGrapheme: Boolean get() {
    ...
    return try {
        ...
        if (iterator.next() != BreakIterator.DONE) false
            ...
            if (tone == -1) true else emoRegex.matches(this) || isEmojiOmittedByLegacyRegex(this)
        }
    } catch (_: Throwable) {
        false
    }
...
val String.lastGrapheme: String get() {
    ...
    return if (tone == -1 || emoRegex.matches(res) || isEmojiOmittedByLegacyRegex(res)) res
