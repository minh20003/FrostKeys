    ...
    fun String.constructLocale(): Locale {
        synchronized(sLocaleCache) {
            ...
            val locale = if (elements.size == 1) {
                ...
                else Locale.Builder().setLanguage(language).setRegion(region.orEmpty()).build()
                ...
                Locale.Builder()
                    ...
                    .setRegion(region.orEmpty())
                    .setScript(elements[2].substringAfter("#"))
                    .build()
            } else {
                Locale.Builder()
                    .setLanguage(language)
                    .setRegion(region.orEmpty())
                    .setVariant(elements[2])
                    .build()
            }
            sLocaleCache[this] = locale
            return locale
