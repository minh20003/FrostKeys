...
public final class RichInputConnection implements PrivateCommandPerformer {
        ...
        if (isConnected()) {
            ...
            if (!Settings.getValues().mInputAttributes.mShouldShowSuggestions && text.length() > 0) {
                ...
                if (lastChar == null || lastChar.length() == 0 || text.charAt(text.length() - 1) != lastChar.charAt(0)) {
                    ...
                    // Editor text is private user input. Lengths are enough to diagnose a
                    // broken InputConnection without retaining composing or surrounding text.
                    Log.w(TAG, "Composing-text verification failed: expected length " + text.length()
                            + ", received length " + (checkedText == null ? -1 : checkedText.length()));
                    return false;
                }
            }
        }
    ...
    public void copyText(final boolean getSelection) {
        ...
        cm.setPrimaryClip(ClipData.newPlainText(mParent.getString(R.string.clipboard_copied_text_label), text));
