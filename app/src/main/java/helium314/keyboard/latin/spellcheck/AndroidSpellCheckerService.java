...
import helium314.keyboard.latin.dictionary.VietnameseDictionaryAssets;
...
public final class AndroidSpellCheckerService extends SpellCheckerService
    ...
    public void onCreate() {
        ...
        try {
            ...
            Log.e(TAG, "Cannot register user dictionary observer", e);
        }
    }
    ...
    public boolean hasMainDictionaryForLocale(final Locale locale) {
        // The service XML can advertise vi before an asset-merge failure is observed. Keep the
        // advertised subtype inert unless its signed offline dictionary and manifest verify.
        if (!VietnameseDictionaryAssets.isBundledMainDictionaryValid(this, locale)) {
            return false;
        }
        mSemaphore.acquireUninterruptibly();
        try {
