...
object SubtypeSettings {
    ...
    fun getSelectedSubtype(prefs: SharedPreferences): InputMethodSubtype {
        ...
        return defaultSubtypes.firstOrNull {
            it.locale() == selectedSubtype.locale
                && it.mainLayoutName() == selectedSubtype.mainLayoutName()
        }
    ...
    fun init(context: Context) {
        ...
        // This object is process-global while the IME service can be created more than once
        // (notably by Robolectric and after the system recreates the service). Rebuild the
        // resource cache instead of appending duplicate subtypes on every initialization.
    ...
    private fun getDefaultEnabledSubtypes(): List<InputMethodSubtype> {
        ...
        if (subtypes.isEmpty()) {
            ...
            val usSubtypes = requireNotNull(resourceSubtypesByLocale[Locale.US]) {
                "The built-in en-US fallback subtype must be registered"
            }
            systemSubtypes.add(usSubtypes.first())
        } else {
    ...
    private fun loadEnabledSubtypes(context: Context) {
        ...
        for (settingsSubtype in settingsSubtypes) {
            ...
            if (subtypesForLocale == null) {
                val message = "An enabled subtype has no matching packaged resource"
                Log.w(TAG, message)
                if (DebugFlags.DEBUG_ENABLED)
                    Toast.makeText(
                        context,
                        context.getString(R.string.debug_subtype_resource_missing, settingsSubtype),
                        Toast.LENGTH_LONG,
                    ).show()
            ...
            val requestedLayout = settingsSubtype.mainLayoutName() ?: SubtypeLocaleUtils.QWERTY
            // A locale + main layout is not a unique identity any more: Telex, VNI and raw
            // Vietnamese all use vi/qwerty. Match the complete persisted subtype first so a
            // restart cannot silently turn VNI/raw back into Telex.
            val subtype = subtypesForLocale.firstOrNull {
                it.toSettingsSubtype() == settingsSubtype
            } ?: run {
                val layoutMatches = subtypesForLocale.filter {
                    it.mainLayoutNameOrQwerty() == requestedLayout
                }
                when {
                    // Upstream Vietnamese settings had no CombiningRules marker. Preserve their
                    // historical meaning as Telex, while never using this fallback for an
                    // explicitly requested VNI/raw subtype.
                    settingsSubtype.locale.language == "vi"
                            && settingsSubtype.getExtraValueOf(ExtraValue.COMBINING_RULES) == null ->
                        layoutMatches.firstOrNull {
                            it.toSettingsSubtype().getExtraValueOf(ExtraValue.COMBINING_RULES) == "vi_telex"
                        }
                    // The old layout-only matcher remains safe only where there is exactly one
                    // declared subtype for that layout.
                    layoutMatches.size == 1 -> layoutMatches.single()
                    else -> null
                }
            }
            if (subtype == null) {
                val message = "An enabled subtype could not be loaded from packaged resources"
                Log.w(TAG, message)
                if (DebugFlags.DEBUG_ENABLED)
                    Toast.makeText(
                        context,
                        context.getString(R.string.debug_subtype_load_failed, settingsSubtype),
                        Toast.LENGTH_LONG,
                    ).show()
                else // don't remove in debug mode
                    removeEnabledSubtype(prefs, settingsSubtype)
    ...
    private fun updateSystemSubtypes(context: Context) {
        try {
            ...
            // Android 12-13 expose the statically declared method.xml subtypes. Android 14
            // added the non-deprecated way to restrict that manifest set. Do not dynamically
            // register arbitrary additional layouts: they are not part of this fixed personal
            // build and the legacy API for doing so has been deprecated since Android 10.
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.UPSIDE_DOWN_CAKE) {
                val manifestHashes = resourceSubtypesByLocale.values
                    .flatten()
                    .mapTo(HashSet()) { it.hashCode() }
                val explicitlyEnabledHashes = getEnabledSubtypes(fallback = true)
                    .asSequence()
                    .map { it.hashCode() }
                    .filter { it in manifestHashes }
                    .distinct()
                    .toList()
                    .toIntArray()
                imm.setExplicitlyEnabledInputMethodSubtypes(imi.id, explicitlyEnabledHashes)
            }
        } catch (e: Exception) {
