...
class GestureDataDao(val db: Database) {
    fun add(data: GestureData, timestamp: Long): Long = synchronized(this) {
    ...
    fun markAsExported(ids: List<Long>, context: Context): Unit = synchronized(this) {
    ...
    fun deleteAll(): Int = synchronized(this) {
        db.writableDatabase.delete(TABLE, null, null)
    }

    fun deletePassiveWords(words: Collection<String>): Int = synchronized(this) {
