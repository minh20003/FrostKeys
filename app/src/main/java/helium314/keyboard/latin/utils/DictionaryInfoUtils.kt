...
object DictionaryInfoUtils {
    ...
    const val ASSETS_DICTIONARY_EXTENSION = ".dict"
    ...
    /**
     * Whether the enabled keyboard languages have an offline source for emoji search.
     *
     * A binary emoji dictionary is optional for Vietnamese: this personal build ships the
     * Unicode CLDR Vietnamese annotations as an offline fallback instead.
     */
    @JvmStatic
    fun hasEmojiSearchData(context: Context): Boolean =
        getLocalesWithEmojiDicts(context).isNotEmpty() ||
            SubtypeSettings.getEnabledSubtypes(true).any {
                CldrEmojiSearchIndex.hasBundledAnnotations(it.locale())
            }
    ...
    fun extractLocaleFromAssetsDictionaryFile(dictionaryFileName: String): Locale {
        require(isAssetsDictionaryFileName(dictionaryFileName)) {
            "Asset dictionary names must use the <type>_<locale>.dict format"
        }
    ...
    /** Phrase models and manifests can share assets/dicts but must never enter locale parsing. */
    internal fun isAssetsDictionaryFileName(fileName: String): Boolean =
        fileName.endsWith(ASSETS_DICTIONARY_EXTENSION, ignoreCase = true)
            && fileName.substringBefore(ASSETS_DICTIONARY_EXTENSION).contains('_')
    ...
    fun extractAssetsDictionary(dictionaryFileName: String, locale: Locale, context: Context): File? {
        ...
        try {
            // Android AssetManager paths are slash-separated even when the host running unit
            // tests is Windows. File.separator made bundled offline dictionaries appear missing
            // to Robolectric and is not a valid asset-path abstraction.
            context.assets.open("$ASSETS_DICTIONARY_FOLDER/$dictionaryFileName").use { inputStream ->
