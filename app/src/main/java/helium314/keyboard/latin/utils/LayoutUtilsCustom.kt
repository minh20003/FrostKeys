    ...
    fun checkKeys(keys: List<List<Key.KeyParams>>): Boolean {
        ...
        if (keys.any { row -> row.any {
                if ((it.mLabel?.length ?: 0) > 20) {
                    Log.w(TAG, "Custom key label exceeds the length limit (${it.mLabel?.length ?: 0} characters)")
                    true
                } else false
            } }) {
            return false
        ...
        if (keys.any { row -> row.any {
                if ((it.mPopupKeys?.size ?: 0) > 20) {
                    Log.w(TAG, "Custom key exceeds the popup-key limit")
                    true
                } else false
            } }) {
            return false
        ...
        if (keys.any { row -> row.any { true == it.mPopupKeys?.any { popupKey ->
                if ((popupKey.mLabel?.length ?: 0) > 10) {
                    Log.w(TAG, "Custom popup-key label exceeds the length limit (${popupKey.mLabel?.length ?: 0} characters)")
                    true
                } else false
            } } }) {
            return false
