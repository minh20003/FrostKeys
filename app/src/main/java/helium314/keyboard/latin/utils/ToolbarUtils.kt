...
fun View.startDragAndDropCompat(
    ...
    localState: Any?,
    flags: Int = 0,
): Boolean = startDragAndDrop(clipData, shadowBuilder, localState, flags)
...
fun setToolbarButtonsActivatedStateOnPrefChange(buttonsGroup: ViewGroup, key: String?) {
    ...
    // Preference callbacks may arrive while the input view is being torn down. Keep the
    // deferred UI work owned by the view instead of a process-wide coroutine.
    buttonsGroup.postDelayed({
        if (!buttonsGroup.isAttachedToWindow) return@postDelayed
