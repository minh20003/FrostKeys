...
object Defaults {
    ...
    const val PREF_TRANSLATION_QUALITY = "fast"  // "fast" or "high"
    const val PREF_TRANSLATION_SOURCE_LANGUAGE = ""  // BCP-47 tag or empty for auto-detect
    const val PREF_TRANSLATION_TARGET_LANGUAGE = "vi"  // BCP-47 tag
    const val PREF_TRANSLATION_ON_DEVICE_FALLBACK = true
    @JvmField
    ...
    // A personal clean install must not silently add keyboards left over from the upstream
    // sample defaults.  Users can explicitly add any extra subtype in Language settings.
    ...
    // Screenshot monitoring is privacy-sensitive and must be an explicit opt-in.
    const val PREF_SHOW_SCREENSHOTS_IN_CLIPBOARD = false
    ...
    @JvmField
    val PREF_NAVBAR_COLOR = true
    const val PREF_NARROW_KEY_GAPS = false
    // A clean personal install is usable immediately: Vietnamese Telex is primary and English
    // remains available for code-mixed text. VNI/raw stay opt-in resource subtypes.
    const val PREF_ENABLED_SUBTYPES = "en-US${Separators.SET}SupportTouchPositionCorrection,TrySuppressingImeSwitcher" +
            "${Separators.SETS}vi${Separators.SET}CombiningRules=vi_telex,KeyboardLayoutSet=MAIN:qwerty"
    const val PREF_SELECTED_SUBTYPE = "vi${Separators.SET}CombiningRules=vi_telex,KeyboardLayoutSet=MAIN:qwerty"
    const val PREF_VIETNAMESE_TONE_PLACEMENT = "common"
    ...
    /** Auto honors power saver, low-RAM devices, and the system animator scale. */
    const val PREF_AI_VISUAL_EFFECTS = "auto"
