...
public class PersonalizationHelper {
    ...
    public static void removeAllUserHistoryDictionaries(final Context context) {
        synchronized (sLangUserHistoryDictCache) {
            ...
            if (!filesDeleted) {
                Log.e(TAG, "Cannot remove user-history dictionary files");
            }
        }
    }

    /**
     * Drops open user-history dictionary handles after a password-protected backup restore.
     * Unlike {@link #removeAllUserHistoryDictionaries(Context)}, this never deletes on-disk data;
     * the next lookup opens the atomically restored dictionary file.
     */
    public static void clearUserHistoryDictionaryCache() {
        synchronized (sLangUserHistoryDictCache) {
            for (final ConcurrentHashMap.Entry<String, SoftReference<UserHistoryDictionary>> entry
                    : sLangUserHistoryDictCache.entrySet()) {
                final SoftReference<UserHistoryDictionary> reference = entry.getValue();
                final UserHistoryDictionary dictionary = reference == null ? null : reference.get();
                if (dictionary != null) {
                    dictionary.close();
                }
            }
