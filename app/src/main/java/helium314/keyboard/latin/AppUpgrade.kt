...
fun checkVersionUpgrade(context: Context) {
    ...
    // Fresh installs must start from code defaults only. Restoring an author-provided backup
    // leaked personal settings, history, fonts and wallpaper URIs into every installation.
    if (oldVersion != BuildConfig.VERSION_CODE)
        AppUpgrade.onUpgrade(context)
}

/**
 * Moves only this fork's legacy pinned text records onto the serialized clipboard IO lane.
 * It never imports another app's data and does not delete the old preference until every entry
 * has been handed to the local DAO. Running it after first unlock avoids creating a database or
 * touching credential-protected state during direct boot.
 */
fun transferOldPinnedClips(context: Context) {
    if (isUserLocked(context) || isDeviceLocked(context)) return
    val applicationContext = context.applicationContext
    ClipboardDao.runSerialized(applicationContext) {
        @Serializable
        ...
        try {
            val preferences = applicationContext.protectedPrefs()
            val encoded = preferences.getString("pinned_clips", null).orEmpty()
            if (encoded.isBlank()) return@runSerialized
            val entries: List<OldClipboardHistoryEntry> = Json.decodeFromString(encoded)
            entries.forEach { entry -> addClip(entry.timeStamp, entry.isPinned, entry.content) }
            preferences.edit { remove("pinned_clips") }
        } catch (error: Throwable) {
            // The legacy record can contain copied text. Do not include the parser/record message.
            Log.e("upgrade", "Could not migrate legacy pinned clipboard entries", error)
        }
    }
}
...
private object AppUpgrade {
    fun onUpgrade(context: Context) {
        ...
        if (oldVersion in 1 until 3_000_001) {
            // 3.0 narrows the personal build to its maintained offline keyboard subtypes. Do this
            // in one preference edit: otherwise an old selected/additional subtype can briefly
            // point to a removed resource subtype while the IME or settings screen is starting.
            val bundledSubtypes = getResourceSubtypes(context.resources).map { it.toSettingsSubtype() }

            fun bundledSubtype(language: String, layout: String? = null, rules: String? = null): SettingsSubtype? =
                bundledSubtypes.firstOrNull { subtype ->
                    subtype.locale.language == language
                            && (layout == null || subtype.mainLayoutName() == layout)
                            && (rules == null || subtype.getExtraValueOf(ExtraValue.COMBINING_RULES) == rules)
                }

            // The platform parser can expose the old `imeSubtypeLocale=en_US` in a
            // resource-only form during an upgrade, even though the resource also has the
            // canonical `languageTag=en-US`.  Keep the persisted setting canonical either way:
            // a removed English regional subtype must never make this personal build lose its
            // one supported English keyboard.
            val englishUs = bundledSubtype("en")
                ?: SettingsSubtype(
                    "en-US".constructLocale(),
                    "KeyboardLayoutSet=MAIN:qwerty",
                )
            val vietnameseTelex = bundledSubtype("vi", layout = "qwerty", rules = "vi_telex")
            val fallbackSelected = vietnameseTelex ?: englishUs

            fun canonicalizePersonalSubtype(pref: String): SettingsSubtype? {
                if (pref.isBlank()) return null
                val saved = pref.toSettingsSubtype()
                // Saved subtype values exist from multiple upstream eras: some use locale
                // specs (`en_GB`), others BCP-47 (`en-GB`). Normalize the persisted prefix
                // directly instead of trusting a platform Locale parsed by an old Android API.
                val persistedLanguage = pref.substringBefore(Separators.SET)
                    .substringBefore('-')
                    .substringBefore('_')
                    .lowercase()
                return when (persistedLanguage) {
                    // English variants in the upstream build all become the one bundled US layout.
                    "en" -> englishUs
                    // The old vi resource subtype was Telex without an explicit combining rule.
                    // Explicit VNI/raw markers from development builds are retained; only an
                    // unmarked Vietnamese preference is interpreted as the legacy Telex choice.
                    "vi" -> when (saved.getExtraValueOf(ExtraValue.COMBINING_RULES)) {
                        "vi_vni" -> bundledSubtype("vi", layout = "qwerty", rules = "vi_vni")
                        "vi_raw" -> bundledSubtype("vi", layout = "qwerty", rules = "vi_raw")
                        else -> vietnameseTelex
                    }
                    "ko" -> bundledSubtype("ko", layout = saved.mainLayoutName())
                        ?: bundledSubtype("ko", layout = "korean")
                    "th" -> bundledSubtype("th", layout = "thai")
                    else -> null
                }
            }

            val enabled = prefs.getString(Settings.PREF_ENABLED_SUBTYPES, Defaults.PREF_ENABLED_SUBTYPES)!!
                .split(Separators.SETS)
                .mapNotNull(::canonicalizePersonalSubtype)
                .toMutableSet()
            if (enabled.isEmpty()) {
                enabled += SubtypeSettings.createSettingsSubtypes(Defaults.PREF_ENABLED_SUBTYPES)
                    .mapNotNull { canonicalizePersonalSubtype(it.toPref()) }
            }

            // A Vietnamese-first fresh install still keeps English available.  Apply the same
            // invariant to upgrades even when an old regional English subtype was already
            // discarded by an earlier migration step.
            enabled += englishUs

            val selected = canonicalizePersonalSubtype(
                prefs.getString(Settings.PREF_SELECTED_SUBTYPE, Defaults.PREF_SELECTED_SUBTYPE)!!
            ) ?: fallbackSelected
            enabled += selected

            prefs.edit {
                // Additional subtypes cannot be guaranteed to have a layout after language assets
                // are pruned, so retain only canonical resources in enabled/selected state.
                putString(Settings.PREF_ADDITIONAL_SUBTYPES, "")
                putString(Settings.PREF_ENABLED_SUBTYPES, SubtypeSettings.createPrefSubtypes(enabled))
                putString(Settings.PREF_SELECTED_SUBTYPE, selected.toPref())

                // Per-app choices are another persistent route into a removed subtype.
                prefs.all.keys
                    .filter { it.startsWith(Settings.PREF_SAVED_APP_SUBTYPE_PREFIX) }
                    .forEach { key ->
                        val saved = prefs.getString(key, null) ?: return@forEach
                        val canonical = canonicalizePersonalSubtype(saved)
                        if (canonical == null) remove(key) else putString(key, canonical.toPref())
                    }
            }
            // App initializes SubtypeSettings before running upgrades; refresh its process-local
            // cache so the current IME session cannot keep a removed subtype until restart.
            SubtypeSettings.reloadEnabledSubtypes(context)
        }
        if (oldVersion > 0
