...
import helium314.keyboard.keyboard.emoji.CldrEmojiSearchIndex;
...
public final class InputLogic {
    ...
    // Vietnamese ships a CLDR-only search index when a binary emoji dictionary is unavailable.
    // The index itself stays lazy; retaining its locale is enough to let ':' enter search mode.
    @Nullable private Locale mCldrEmojiSearchLocale;
    ...
    public void startInput(final String combiningSpec, final SettingsValues settingsValues) {
        ...
        // Telex/VNI sequences are intentionally disabled in credentials, URI/email and numeric
        // fields. Those inputs commonly contain literal trigger letters and VNI digits.
        final boolean vietnameseCombining = "vi_telex".equals(combiningSpec)
                || "vi_vni".equals(combiningSpec);
        final String effectiveCombiningSpec = vietnameseCombining
                && !settingsValues.mInputAttributes.mIsGeneralTextInput ? "" : combiningSpec;
        mWordComposer.restartCombining(effectiveCombiningSpec);
    ...
    private void enterInlineEmojiSearchIfNeeded(int codePoint, SettingsValues settingsValues) {
        if (!hasInlineEmojiSearchSource() || isInlineEmojiSearchAction()) {
            return;
        }
    ...
    private void searchForEmojiInline(int sequenceNumber, OnGetSuggestedWordsCallback callback) {
        ...
        var typedWordInfo = new SuggestedWordInfo(input, "", SuggestedWordInfo.MAX_SCORE, SuggestedWordInfo.KIND_TYPED,
                                 Dictionary.DICTIONARY_USER_TYPED, SuggestedWordInfo.NOT_AN_INDEX, SuggestedWordInfo.NOT_A_CONFIDENCE);
        var suggestedWordInfos = new ArrayList<SuggestedWordInfo>();
        suggestedWordInfos.add(typedWordInfo);

        ArrayList<SuggestedWordInfo> rawSuggestions = null;
        if (mEmojiDictionaryFacilitator != null) {
            ...
            rawSuggestions = suggestions.mRawSuggestions;
        ...
        } else if (mCldrEmojiSearchLocale != null) {
            // Keep the same result cap as the binary dictionary path. CLDR preserves the build's
            // offline/no-network invariant and is used only when there is no binary emoji dict.
            int rank = 0;
            for (final String emoji : CldrEmojiSearchIndex.search(mCldrEmojiSearchLocale, input)) {
                if (rank >= SuggestedWords.MAX_SUGGESTIONS) {
                    break;
                }
                final SuggestedWordInfo suggestion = new SuggestedWordInfo(
                        emoji, "", SuggestedWordInfo.MAX_SCORE - rank,
        ...
        // Preserve the binary dictionary path's historical typed-word behavior. The CLDR path
        // has no candidate until its lazy index is ready (or the query does not match), so it
        // must instead report an empty result.
        if (mEmojiDictionaryFacilitator == null && suggestedWordInfos.size() == 1) {
            callback.onGetSuggestedWords(SuggestedWords.getEmptyInstance());
            return;
        }
        callback.onGetSuggestedWords(new SuggestedWords(suggestedWordInfos, rawSuggestions, typedWordInfo,
    ...
    public void updateEmojiDictionary(Locale locale) {
        if (Settings.getValues().mInlineEmojiSearch && Settings.getValues().needsToLookupSuggestions() && ! mLatinIME.isEmojiSearch()) {
            ...
            if (mEmojiDictionaryFacilitator == null && CldrEmojiSearchIndex.hasBundledAnnotations(locale)) {
                mCldrEmojiSearchLocale = locale;
                CldrEmojiSearchIndex.warm(mLatinIME, locale);
            } else {
                // A binary dictionary remains authoritative when supplied by the user.
                mCldrEmojiSearchLocale = null;
            }
        } else {
            closeEmojiDictionary();
        }
    ...
    private boolean hasInlineEmojiSearchSource() {
        return mEmojiDictionaryFacilitator != null || mCldrEmojiSearchLocale != null;
    }
