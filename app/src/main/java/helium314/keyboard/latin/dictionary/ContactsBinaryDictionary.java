...
public class ContactsBinaryDictionary extends ExpandableBinaryDictionary
    ...
    private void loadDictionaryForUriLocked(final Uri uri) {
        if (!mContactsManager.isContactSuggestionsAccessEnabled()) {
            Log.i(TAG, "Contact suggestions are disabled or unavailable. Not loading the dictionary.");
            return;
        }
    ...
    private void addNameLocked(final String name) {
        ...
        for (int i = 0; i < len; i++) {
            if (Character.isLetter(name.codePointAt(i))) {
                ...
                if (DEBUG_DUMP) {
                    Log.d(TAG, "Adding a contacts dictionary token");
                }
                i = end - 1;
                ...
                if (wordLen <= MAX_WORD_LENGTH && wordLen > 1) {
                    if (DEBUG) {
                        Log.d(TAG, "Adding a contacts dictionary entry");
                    }
    ...
    public int getFrequency(final String word) {
        return mContactsManager.isContactSuggestionsAccessEnabled()
                ? super.getFrequency(word) : NOT_A_PROBABILITY;
    }

    @Override
    public ArrayList<SuggestedWordInfo> getSuggestions(
            final helium314.keyboard.latin.common.ComposedData composedData,
            final NgramContext ngramContext, final long proximityInfoHandle,
            final helium314.keyboard.latin.settings.SettingsValuesForSuggestion settingsValuesForSuggestion,
            final int sessionId, final float weightForLocale,
            final float[] inOutWeightOfLangModelVsSpatialModel) {
        return mContactsManager.isContactSuggestionsAccessEnabled()
                ? super.getSuggestions(composedData, ngramContext, proximityInfoHandle,
    ...
    public boolean isInDictionary(final String word) {
        return mContactsManager.isContactSuggestionsAccessEnabled() && super.isInDictionary(word);
    }

    @Override
    public int getMaxFrequencyOfExactMatches(final String word) {
        return mContactsManager.isContactSuggestionsAccessEnabled()
                ? super.getMaxFrequencyOfExactMatches(word) : NOT_A_PROBABILITY;
    }
}
