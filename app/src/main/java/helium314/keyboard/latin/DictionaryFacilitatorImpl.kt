...
import helium314.keyboard.latin.dictionary.VietnamesePhraseModelAssets
import helium314.keyboard.latin.dictionary.VietnamesePhraseModelData
...
class DictionaryFacilitatorImpl : DictionaryFacilitator {
    ...
    // This is an immutable APK asset, never user history. It is loaded on a background dispatcher
    // after a Vietnamese dictionary reset and only consulted for blank-composition predictions.
    @Volatile
    private var vietnamesePhraseModel: VietnamesePhraseModelData? = null
    ...
    override fun resetDictionaries(
        ...
        refreshVietnamesePhraseModel(context, locales)
    ...
    override fun getSuggestionResults(
        ...
        addVietnamesePhrasePredictions(suggestionResults, composedData, ngramContext)
    ...
    /**
     * Adds only deterministic next-word predictions. It never participates in correction,
     * gesture decoding, or auto-commit because [composedData] must be blank. A verified main
     * Vietnamese dictionary is also required, so a phrase asset cannot make a broken dictionary
     * look usable by itself.
     */
    private fun addVietnamesePhrasePredictions(
        suggestionResults: SuggestionResults,
        composedData: ComposedData,
        ngramContext: NgramContext,
    ) {
        if (composedData.mIsBatchMode || composedData.mTypedWord.isNotEmpty()) return
        val primaryGroup = dictionaryGroups.firstOrNull() ?: return
        if (!primaryGroup.locale.language.equals("vi", ignoreCase = true)) return
        if (primaryGroup.getDict(Dictionary.TYPE_MAIN)?.isInitialized != true) return
        val model = vietnamesePhraseModel ?: return
        val predictions = model.predict(ngramContext.extractPrevWordsContextArray())
        for (prediction in predictions) {
            if (suggestionResults.any { it.word.equals(prediction.word, ignoreCase = true) }) continue
            // Give the curated contextual result a higher rank than a generic next-word result,
            // but it remains a KIND_PREDICTION and is therefore never eligible for auto-commit.
            val info = SuggestedWordInfo(
                prediction.word,
                ngramContext.extractPrevWordsContext(),
                VIETNAMESE_PHRASE_PREDICTION_SCORE_BASE + prediction.score,
                SuggestedWordInfo.KIND_PREDICTION,
                Dictionary.DICTIONARY_VIETNAMESE_PHRASE_MODEL,
                SuggestedWordInfo.NOT_AN_INDEX,
                SuggestedWordInfo.NOT_A_CONFIDENCE,
            )
            suggestionResults.add(info)
            suggestionResults.mRawSuggestions?.add(info)
        }
    }

    private fun refreshVietnamesePhraseModel(context: Context, locales: Collection<Locale>) {
        vietnamesePhraseModel = null
        if (locales.none { it.language.equals("vi", ignoreCase = true) }) return
        scope.launch {
            // The loader only opens and verifies an APK asset. There is deliberately no remote
            // fallback: an absent/corrupt model simply leaves normal dictionary suggestions alone.
            vietnamesePhraseModel = VietnamesePhraseModelAssets.load(context, Locale.forLanguageTag("vi"))
        }
    }
    ...
    companion object {
        ...
        private const val VIETNAMESE_PHRASE_PREDICTION_SCORE_BASE = 10_000
...
private class DictionaryGroup(
    ...
    fun removeWord(word: String) {
        ...
        if (mainDict.isValidWord(lowercase)) {
        ...
        scope.launch {
            synchronized(this) {
                try {
                    ...
                    // Blacklisted words are user data; identify only the storage operation.
                    Log.e(TAG, "Exception while adding a word to blacklist ${blacklistFile.name}", e)
                }
            }
        }
    }
        ...
        scope.launch {
            synchronized(this) {
                try {
                    ...
                    // Blacklisted words are user data; identify only the storage operation.
                    Log.e(TAG, "Exception while removing a word from blacklist ${blacklistFile.name}", e)
                }
            }
        }
    }
