...
class Event private constructor(
    ...
    // State of meta keys (currently shift, ctrl, alt, fn, meta)
    ...
    val isFunctionalKeyEvent: Boolean
        get() = NOT_A_CODE_POINT == codePoint || (metaState and TEXT_INPUT_SAFE_META_STATES.inv()) != 0
    ...
    companion object {
        ...
        // Shift and Caps Lock modify a printable character returned by a hardware keyboard; they
        // do not turn that character into a command. In particular, Vietnamese Telex/VNI must
        // still receive Shift+D so `Dd` becomes `Đ`. Ctrl/Alt/Meta/Fn/Sym remain functional.
        private const val TEXT_INPUT_SAFE_META_STATES =
            KeyEvent.META_SHIFT_ON or KeyEvent.META_SHIFT_LEFT_ON or KeyEvent.META_SHIFT_RIGHT_ON or
