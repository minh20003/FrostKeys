...
fun createPreferencesSettings(context: Context) = listOf(
    ...
    Setting(
        context,
        Settings.PREF_VIETNAMESE_TONE_PLACEMENT,
        R.string.vietnamese_tone_placement_title,
        R.string.vietnamese_tone_placement_summary
    ) {
        ListPreference(
            it,
            listOf(
                stringResource(R.string.vietnamese_tone_placement_common) to "common",
                stringResource(R.string.vietnamese_tone_placement_alternate) to "alternate"
            ),
        ...
        R.string.enable_clipboard_history, R.string.enable_clipboard_history_summary)
    {
        ...
        SwitchPreference(it, Defaults.PREF_ENABLE_CLIPBOARD_HISTORY) {
            ClipboardDao.runSerialized(ctx) { clearNonPinned() }
        }
    },
    ...
    Setting(context, Settings.PREF_CLIPBOARD_HISTORY_RETENTION_TIME, R.string.clipboard_history_retention_time) { setting ->
        ...
        SliderPreference(
            ...
            range = 1f..121f,
        ) { ClipboardDao.runSerialized(ctx) { clearOldClips(true) } }
    },
...
private fun ScreenshotClipboardPreference(setting: Setting) {
    ...
    val prefs = ctx.prefs()
    val permissions = PermissionsUtil.getScreenshotReadPermissionsToRequest()
    var permissionGranted by remember {
        mutableStateOf(PermissionsUtil.hasFullScreenshotReadPermission(ctx))
    }
    ...
    val launcher = rememberLauncherForActivityResult(ActivityResultContracts.RequestMultiplePermissions()) {
        // Android 14 can report READ_MEDIA_IMAGES as granted for a selected-photo-only session.
        // Re-check the full access state instead of trusting the contract result.
        permissionGranted = PermissionsUtil.hasFullScreenshotReadPermission(ctx)
    ...
    fun switchTo(newValue: Boolean) {
        if (newValue && permissions.isNotEmpty() && !permissionGranted) {
