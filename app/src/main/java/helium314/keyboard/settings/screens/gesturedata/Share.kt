...
fun ShareGestureData(ids: List<Long>) {
    ...
    if (!exportDone) {
        ...
        ButtonWithText(stringResource(R.string.gesture_data_copy_mail)) {
            scope.launch {
                clip.setClipEntry(ClipEntry(ClipData.newPlainText(
                    resources.getString(R.string.gesture_data_mail_address_label),
                    deobfuscateEmail(ctx),
                )))
            }
        }
...
private fun createSendIntentChooser(context: Context): Intent {
    ...
    val shareFileIntent = Intent(Intent.ACTION_SEND).apply {
        ...
        putExtra(Intent.EXTRA_SUBJECT, context.getString(R.string.gesture_data_mail_subject, BuildConfig.VERSION_NAME))
    ...
    val chooser = Intent.createChooser(shareFileIntent, context.getString(R.string.gesture_data_share_chooser)).apply {
        addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION)
