        ...
        title = {
            Column {
                ...
                androidx.compose.material3.TextButton(
                    ...
                    Text(
                        stringResource(R.string.sync_enabled_languages_with_os_switcher),
    ...
    Row(
        ...
        Column(modifier = Modifier.weight(1f)) {
            ...
            val descriptions = buildList {
                if (SubtypeSettings.isAdditionalSubtype(subtype)) {
                    val secondaryLocales = subtype.getExtraValueOf(ExtraValue.SECONDARY_LOCALES)?.split(Separators.KV)
                        ?.joinToString(", ") { it.constructLocale().localizedDisplayName(resources) }
                    add(stringResource(R.string.custom_subtype) + (secondaryLocales?.let { "\n$it" } ?: ""))
                }
                if (subtype.extraValue.contains("CombiningRules=vi_vni"))
                    add(stringResource(R.string.vietnamese_vni_number_row_note))
            }
...
private fun getSortedSubtypes(context: Context): List<InputMethodSubtype> {
    ...
    val defaultAdditionalSubtypes = Defaults.PREF_ADDITIONAL_SUBTYPES
        .split(Separators.SETS)
        .filter { it.isNotBlank() }
        .map {
            it.substringBefore(Separators.SET) to
                (it.substringAfter(Separators.SET) + ",AsciiCapable,EmojiCapable,isAdditionalSubtype")
        }
    ...
    val subtypeSortComparator = compareBy<InputMethodSubtype>(
        ...
        { it.languageTag == SubtypeLocaleUtils.NO_LANGUAGE },
        { it.displayName() }
    )
