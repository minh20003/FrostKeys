...
fun createAdvancedSettings(context: Context) = listOf(
    ...
    Setting(context, Settings.PREF_EMOJI_MAX_SDK, R.string.prefs_key_emoji_max_sdk) { setting ->
        ...
        SliderPreference(
            ...
            stepSize = 1,
            description = {
                val baseDescription = resources.getString(R.string.emoji_android_version, emojiAndroidVersionName(
                    it,
                    resources.getString(R.string.emoji_version_unknown),
                ))
                val dynamicMax = SupportedEmojis.getDynamicMaxSdk(ctx)
                if (it > dynamicMax) {
                    val supportedName = emojiAndroidVersionName(
                        dynamicMax,
                        resources.getString(R.string.emoji_current_version),
                    )
                    baseDescription + "\n" + resources.getString(
                        R.string.emoji_unsupported_warning,
                        supportedName,
                    )
                } else {
                    baseDescription
...
private fun emojiAndroidVersionName(apiLevel: Int, fallback: String): String = when (apiLevel) {
    21 -> "5.0"
    22 -> "5.1"
    23 -> "6"
    24 -> "7.0"
