...
fun ColorThemePickerDialog(
    ...
    val loadFilePicker = filePicker { uri ->
        ctx.getActivity()?.contentResolver?.openInputStream(uri)?.use {
            ...
            scope.launch { errorDialog = !loadColorString(text, prefs, ctx) }
        }
    }
    if (showLoadDialog) {
        ConfirmationDialog(
            ...
            onNeutral = {
                ...
                errorDialog = !loadColorString(text.toString(), prefs, ctx)
            },
...
private fun UnsupportedFrostedGlassBlurDialog(onDismissRequest: () -> Unit) {
    val deviceName = listOf(Build.MANUFACTURER, Build.MODEL)
        ...
        .joinToString(" ")
    val fallbackDeviceName = stringResource(R.string.frosted_blur_unsupported_device)
    ThreeButtonAlertDialog(
        onDismissRequest = onDismissRequest,
        onConfirmed = { },
        confirmButtonText = null,
        ...
        content = {
            Text(stringResource(
                R.string.frosted_glass_blur_unsupported_message,
                deviceName.ifBlank { fallbackDeviceName },
            ))
        },
        scrollContent = true
    )
...
private fun loadColorString(colorString: String, prefs: SharedPreferences, context: Context): Boolean {
    try {
        val that = Json.decodeFromString<SaveThoseColors>(colorString)
        val themeName = KeyboardTheme.getUnusedThemeName(
            that.name ?: context.getString(R.string.theme_imported_colors),
            prefs,
        )
        ...
        try {
            ...
            var themeName = context.getString(R.string.theme_imported_colors)
