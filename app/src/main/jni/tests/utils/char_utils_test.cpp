...
TEST(CharUtilsTest, VietnamesePrecomposedLettersUseRecursiveBases) {
    // Verify every precomposed Vietnamese letter, including letters which otherwise decompose
    // through another accented letter (for example, ấ -> â -> a).
    const struct {
        int first;
        int last;
        int upper;
    ...
    for (const auto &range : ranges) {
        for (int c = range.first; c <= range.last; ++c) {
            const int expected = (c & 1) == 0 ? range.upper : range.lower;
            EXPECT_EQ(expected, CharUtils::toBaseCodePoint(c)) << std::hex << c;
            EXPECT_EQ(range.lower, CharUtils::toBaseLowerCase(c)) << std::hex << c;
        }
    }
    EXPECT_EQ('D', CharUtils::toBaseCodePoint(0x0110 /* Đ */));
    EXPECT_EQ('d', CharUtils::toBaseCodePoint(0x0111 /* đ */));
    EXPECT_EQ('d', CharUtils::toBaseLowerCase(0x0110 /* Đ */));
    EXPECT_EQ('d', CharUtils::toBaseLowerCase(0x0111 /* đ */));
}
