...
f""", // no newline at the end
    ...
    @Test fun arabicScriptDeleteKeyMatchesLetterWidth() {
        // Farsi is no longer an advertised personal-build subtype, but keep this parser
        // regression test independent of method.xml while its layout asset remains covered.
        val subtype = SubtypeUtilsAdditional.createAdditionalSubtype(
            Locale("fa"),
            "KeyboardLayoutSet=MAIN:farsi,NoShiftKey,SupportTouchPositionCorrection",
            false,
            true,
        )
    ...
    @Test fun urduPakistanDeleteKeyKeepsFunctionalWidth() {
        // As above, exercise the legacy layout parser without re-registering Urdu as an IME subtype.
        val subtype = SubtypeUtilsAdditional.createAdditionalSubtype(
            Locale("ur", "PK"),
            "KeyboardLayoutSet=MAIN:urdu|SYMBOLS:symbols_arabic",
            false,
            true,
        )
        val (_, rows) = buildKeyboard(EditorInfo(), subtype, KeyboardId.ELEMENT_ALPHABET)
