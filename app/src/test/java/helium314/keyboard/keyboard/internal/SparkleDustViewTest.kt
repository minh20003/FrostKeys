...
class SparkleDustViewTest {
    ...
    @Test
    fun hidingViewStopsTheAnimationLoop() {
        val view = createLaidOutDustView()

        view.startDustAnimation()
        view.visibility = View.GONE

        // Drawing directly bypasses normal ViewGroup visibility filtering, so this proves the
        // view itself stopped producing particles rather than merely being clipped by its parent.
        assertFalse(view.isDustAnimationRunningForTest())
        assertFalse(renderToBitmap(view).hasPaintedPixels())
    }

    @Test
    fun zeroParticleBudgetCannotStartAnInvisibleFrameLoop() {
        val view = createLaidOutDustView()

        view.setParticleBudget(0)
        view.startDustAnimation()
