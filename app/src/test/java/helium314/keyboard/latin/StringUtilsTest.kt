...
class StringUtilsTest {
    ...
    @Test fun isEmojiDetectsAllAvailableEmojis() {
        ...
        val allEmojis = ctx.assets.list("emoji")!!.filter { it.endsWith(".txt") }.flatMap {
        ...
        allEmojis.forEach {
            ...
            assert(isEmoji(it)) { "Failed to identify bundled emoji: $it (${it.codePoints().toArray().joinToString { cp -> "U+%04X".format(cp) }})" }
