...
class InputLogicTest {
    ...
    @Test fun vietnameseCursorMoveFinishesCompositionBeforeEditingInsideTheWord() {
        reset()
        val telex = SubtypeSettings.getResourceSubtypesForLocale("vi".constructLocale())
            .single { it.extraValue.contains("CombiningRules=vi_telex") }
        latinIME.switchToSubtype(telex)
        chainInput("tieengs")
        assertEquals("tiếng", text)
        assertEquals("tiếng", composingText)

        // Cursor after "ti". Vietnamese composition must be finalized before the editor edits
        // inside it; otherwise the next key would be appended to the stale composing buffer.
        setCursorPosition(2)
        input('b')
        assertEquals("tibếng", text)
        assertEquals(3, getCursorPosition())
    }

    @Test fun vietnameseSelectionFinishesCompositionBeforeReplacingSelectedText() {
        reset()
        val telex = SubtypeSettings.getResourceSubtypesForLocale("vi".constructLocale())
            .single { it.extraValue.contains("CombiningRules=vi_telex") }
        latinIME.switchToSubtype(telex)
        ...
        // Selecting part of the composing word must discard the retained Telex buffer before
        // the next key replaces the editor selection. Otherwise the next key is appended to the
        // old syllable at its former end.
        setCursorPosition(2, 5)
        input('b')
        assertEquals("tib", text)
        assertEquals(3, getCursorPosition())
    }

    @Test fun vietnameseFreeEndOfWordMarkersReachTheEditorAndCommitNormally() {
        reset()
        val telex = SubtypeSettings.getResourceSubtypesForLocale("vi".constructLocale())
            .single { it.extraValue.contains("CombiningRules=vi_telex") }
        latinIME.switchToSubtype(telex)
        ...
        chainInput("ddangwf")
        assertEquals("đằng", text)
        assertEquals("đằng", composingText)
        input(' ')
        assertEquals("đằng ", text)
        assertEquals("", composingText)

        reset()
        val vni = SubtypeSettings.getResourceSubtypesForLocale("vi".constructLocale())
            .single { it.extraValue.contains("CombiningRules=vi_vni") }
        latinIME.switchToSubtype(vni)
        chainInput("duong9772")
        assertEquals("đường", text)
        assertEquals("đường", composingText)
    }

    @Test fun vietnameseTriggersRemainLiteralInCredentialsUriEmailAndNumericFields() {
        reset()
        val telex = SubtypeSettings.getResourceSubtypesForLocale("vi".constructLocale())
            .single { it.extraValue.contains("CombiningRules=vi_telex") }
        latinIME.switchToSubtype(telex)
        ...
        ).forEach { protectedInputType ->
            setInputType(protectedInputType)
            setText("")
            chainInput("tieengs")
            assertEquals("tieengs", text)
        }
...
//        assertEquals("example.cz", composingText) // fails, but probably would be better than above
    ...
    private fun reset() {
        ...
        latinIME.prefs().edit(commit = true) {
            clear()
            putString(Settings.PREF_ENABLED_SUBTYPES, TEST_ENGLISH_SUBTYPE)
            putString(Settings.PREF_SELECTED_SUBTYPE, TEST_ENGLISH_SUBTYPE)
        }
        // These assertions cover the generic Latin InputLogic contract.  The product's clean
        // install deliberately starts in Vietnamese Telex, so select English explicitly rather
        // than accidentally coupling hundreds of unrelated editor tests to that user default.
...
private const val TEST_ENGLISH_SUBTYPE = "en-US§SupportTouchPositionCorrection,TrySuppressingImeSwitcher"
